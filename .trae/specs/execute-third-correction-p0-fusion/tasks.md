...
- [ ] Task 4.1: 执行阶段启动审计和风险门禁
  - [ ] SubTask 4.1.1: 对每个阶段执行启动前四问审计
  - [ ] SubTask 4.1.2: 对发现的高风险事项发起确认并记录决策
  - [ ] SubTask 4.1.3: 启动审计结论为通过后才执行对应阶段

- [ ] Task 4.2: 执行累积验证
  - [ ] SubTask 4.2.1: 验证知识库文档完整性与链接回路
  - [ ] SubTask 4.2.2: 验证文档+计划+审计体系集成一致性
  - [ ] SubTask 4.2.3: 验证 P0 修复项独立功能
  - [ ] SubTask 4.2.4: 验证 P0 修复项与既有桌面端、附件、World、聊天历史集成

- [ ] Task 4.3: 执行后端验收审计
  - [ ] SubTask 4.3.1: 对本轮代码差异执行代码质量审查
  - [ ] SubTask 4.3.2: 对无法静态确认的问题执行运行时调试与证据采集
  - [ ] SubTask 4.3.3: 对本轮新增/修改输入边界、文件处理、权限控制和数据暴露执行安全审查

- [ ] Task 4.4: 执行前端真实体验审计
  - [ ] SubTask 4.4.1: 使用真实 Electron 窗口启动隔离实例
  - [ ] SubTask 4.4.2: 枚举聊天、附件、动态岛、World、知识相关可交互元素
  - [ ] SubTask 4.4.3: 采集控制台、网络、截图、元素状态和字符清单
  - [ ] SubTask 4.4.4: 记录所有失败项并生成整改任务

- [ ] Task 4.5: 完成交付物索引与最终报告
  - [ ] SubTask 4.5.1: 汇总信息总览知识库路径
  - [ ] SubTask 4.5.2: 汇总第三次修正计划文档路径
  - [ ] SubTask 4.5.3: 汇总阶段启动/验收审计记录路径
  - [ ] SubTask 4.5.4: 汇总累积验证报告路径
  - [ ] SubTask 4.5.5: 汇总决策记录路径
  - [ ] SubTask 4.5.6: 汇总向量知识库连接尝试结果
