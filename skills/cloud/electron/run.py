"""electron skill (cloud) — Electron 自动化 / Electron.
...
def run(args: dict) -> dict:
    """Skill entry point. ``args`` is a free-form dict; key: 'app_path'."""
    args = args or {}
    app_path_value = args.get("app_path")
    if not app_path_value:
        return {"error": "missing app_path", "provider_hint": PROVIDER_HINT}
    ...
    if not True  # 无需凭据:
        return {
            ...
            "read_only": READ_ONLY,
            "app_path": str(app_path_value)[:80],
        }
