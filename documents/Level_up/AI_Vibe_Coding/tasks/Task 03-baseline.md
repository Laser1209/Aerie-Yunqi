...
- 本批定向回归：`28 passed`；相关修改文件诊断为空。
- 历史回归：Phase 0 + Phase 2 + Phase 3 共 `51 passed, 4 warnings`；完整 Python `334 passed, 6 warnings`。
- 未完成：真实库 backup/dry-run/cursor 守恒报告、`conversation_model_v1` 关闭后的真实组合根回滚、完整 Turn 读取与 Pipeline 兼容切换；不得将本 Task 标记为 done。
