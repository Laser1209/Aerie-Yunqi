    ...
    <PageShell videoSrc={publicPath('videos/download.mp4')} accent="#d7eeff" fallbackColor="#080b10" scrollable>
      <div className="flex min-h-screen flex-col overflow-x-hidden px-5 pb-16 pt-28 sm:px-6 md:px-12 lg:px-20 lg:pb-24">
        <PageIntro
          ...
          description="Aerie Companion 当前以 Windows 内测版本提供。下载到本机，连接你选择的模型 Provider；QQ 能力按需配置 NapCat。"
        />
        ...
        <div className="mt-14 grid min-w-0 grid-cols-1 gap-5 lg:mt-20 lg:grid-cols-[minmax(0,1.25fr)_minmax(310px,0.75fr)]">
          ...
          >
            ...
            <div className="mt-auto pt-14">
              ...
              <h2 className="mt-3 font-heading text-4xl italic leading-none md:text-6xl">Aerie Companion {release.version}</h2>
              <p className="mt-5 max-w-xl text-sm font-light leading-relaxed text-white/70">
                便携版适合直接放进自己的工作目录；Windows Setup 则提供标准安装流程。两种真实构建均来自当前 A04 发布基线；公开主线源码可能包含更新的 feature-gated 能力。
              </p>
                ...
                {historicalReleases.map((item) => (
                  <div key={item.version} className="flex flex-wrap items-center justify-between gap-4 py-5 first:pt-0 last:pb-0">
                    <div className="min-w-0">
                      <p className="font-heading text-2xl italic leading-none">Aerie Companion {item.version}</p>
