...
- **主动发图话题轮换**（`companion._run_proactive_photo_loop`）：修复"永远只取 topics[0] 导致被 4h 去重窗口卡死、主动发图静默停摆"的 bug——现在 topics[0] 去重命中会轮换到后续话题（coffee_break/物件话题等），全被去重才跳过本轮
- **世界时钟时区加固**（`world_simulation.tick`）：无论时钟返回 naive 还是 aware（含 UTC），一律 `astimezone(LOCAL_TZ)` 归一化后再判 phase/energy/iso_time，杜绝"UTC 时钟把 18:26 误判成 morning、世界整天卡在 morning_plan"的复发
