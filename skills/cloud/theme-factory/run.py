"""theme-factory skill (cloud) — 主题工厂 / Theme factory.
...
def run(args: dict) -> dict:
    """Skill entry point. ``args`` is a free-form dict; key: 'artifact'."""
    args = args or {}
    artifact_value = args.get("artifact")
    if not artifact_value:
        return {"error": "missing artifact", "provider_hint": PROVIDER_HINT}
    ...
    if not True:  # 无需凭据
        return {
            ...
            "read_only": READ_ONLY,
            "artifact": str(artifact_value)[:80],
        }
