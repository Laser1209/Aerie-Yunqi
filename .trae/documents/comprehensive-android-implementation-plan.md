...
> 整合范围：Android Master Plan（[Aerie_Android_Companion_Master_Plan.md](../../documents/Android/Aerie_Android_Companion_Master_Plan.md)）剩余阶段。用户已确认：范围=安卓端路线；优先级=文件→Tunnel→发布；硬性截止 2026-09-01。
...
- 服务器仓库 `main` 分支有 6 个 ahead 未推送提交，工作树含运行态文件（chroma/data/desire_state 等），开发过程需保持白名单纪律。
...
- `ui/AerieApp.kt`：文件 tab（`selectedIndex == 2`）替换占位为 `FileScreen`：
  ...
  - 下载后打开（`FileProvider` + Intent.ACTION_VIEW，或临时目录）
...
| **S6 签名发布** | keystore、签名 APK、覆盖安装、QQ 独立验收 | 08-27 ~ 08-29 | S5 | 签名 APK 验收、数据保留、AERIE_DISABLE_QQ 闭环 |
...
| R5 | Room v3→v4 迁移破坏既有聊天数据 | 低/高 | 沿用 MIGRATION_1_2/2_3 模式，只 ADD TABLE；迁移测试 + 真机覆盖安装验证 1188 条保留 |
...
- 每阶段先跑定向测试，再跑相关回归；数据库变更必须备份 + dry-run + quick_check。
