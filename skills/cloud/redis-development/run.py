"""redis-development skill (cloud) — Redis 开发 / Redis.

Block-5B scaffold. Cloud-based skill that requires external
credentials (env: `REDIS_URL`). When the env var is
missing we return a stub so the main pipeline is never broken.
The actual HTTP/sign/SDK call is intentionally a TODO until
the corresponding cloud account is provisioned.
"""
from __future__ import annotations
import logging
import os
logger = logging.getLogger(__name__)

PROVIDER_HINT = "text"
READ_ONLY = True
ENV_VAR = 'REDIS_URL'


def run(args: dict) -> dict:
    """Skill entry point. ``args`` is a free-form dict; key: 'query'."""
    args = args or {}
    query_value = args.get("query")
    if not query_value:
        return {"error": "missing query", "provider_hint": PROVIDER_HINT}

    if not os.getenv('REDIS_URL'):
        return {
            "status": "stub",
            "error": f"credential_missing: env {ENV_VAR!r} not set",
            "provider_hint": PROVIDER_HINT,
            "read_only": READ_ONLY,
            "query": str(query_value)[:80],
        }

    # TODO: actual cloud call
    return {
        "status": "stub",
        "error": "cloud_call_not_implemented",
        "provider_hint": PROVIDER_HINT,
        "read_only": READ_ONLY,
        "env": ENV_VAR,
        "note": "real SDK call pending cloud account provisioning",
    }
