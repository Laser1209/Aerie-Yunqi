"""byted-mediakit skill (cloud) — 字节 mediakit 多媒体处理 / ByteDance mediakit.

Block-5B scaffold. Cloud-based skill that requires external
credentials (env: `MEDIAKIT_AK`). When the env var is
missing we return a stub so the main pipeline is never broken.
The actual HTTP/sign/SDK call is intentionally a TODO until
the corresponding cloud account is provisioned.
"""
from __future__ import annotations
import logging
import os
logger = logging.getLogger(__name__)

PROVIDER_HINT = "text"
READ_ONLY = False
ENV_VAR = 'MEDIAKIT_AK'


def run(args: dict) -> dict:
    """Skill entry point. ``args`` is a free-form dict; key: 'input_path'."""
    args = args or {}
    input_path_value = args.get("input_path")
    if not input_path_value:
        return {"error": "missing input_path", "provider_hint": PROVIDER_HINT}

    if not os.getenv('MEDIAKIT_AK'):
        return {
            "status": "stub",
            "error": f"credential_missing: env {ENV_VAR!r} not set",
            "provider_hint": PROVIDER_HINT,
            "read_only": READ_ONLY,
            "input_path": str(input_path_value)[:80],
        }

    # TODO: actual cloud call
    return {
        "status": "stub",
        "error": "cloud_call_not_implemented",
        "provider_hint": PROVIDER_HINT,
        "read_only": READ_ONLY,
        "env": ENV_VAR,
        "note": "real SDK call pending cloud account provisioning",
    }
