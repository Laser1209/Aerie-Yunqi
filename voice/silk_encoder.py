...
QQ 引擎 requires Silk-encoded audio for voice messages.
...
def wav_to_silk(wav_path: Path, silk_path: Optional[Path] = None) -> Optional[Path]:
    ...
    QQ 引擎 supports sending Silk-encoded audio via record messages.
    ...
    # we keep the raw PCM for the engine to handle.
    ...
    # Fallback: keep PCM as-is and let the engine handle the encoding
