"""tianyan skill (cloud) — 天眼查企业信息 / Tianyancha.

Block-5B scaffold. Cloud-based skill that requires external
credentials (env: `TIANYAN_TOKEN`). When the env var is
missing we return a stub so the main pipeline is never broken.
The actual HTTP/sign/SDK call is intentionally a TODO until
the corresponding cloud account is provisioned.
"""
from __future__ import annotations
import logging
import os
logger = logging.getLogger(__name__)

PROVIDER_HINT = "text"
READ_ONLY = True
ENV_VAR = 'TIANYAN_TOKEN'


def run(args: dict) -> dict:
    """Skill entry point. ``args`` is a free-form dict; key: 'company_name'."""
    args = args or {}
    company_name_value = args.get("company_name")
    if not company_name_value:
        return {"error": "missing company_name", "provider_hint": PROVIDER_HINT}

    if not os.getenv('TIANYAN_TOKEN'):
        return {
            "status": "stub",
            "error": f"credential_missing: env {ENV_VAR!r} not set",
            "provider_hint": PROVIDER_HINT,
            "read_only": READ_ONLY,
            "company_name": str(company_name_value)[:80],
        }

    # TODO: actual cloud call
    return {
        "status": "stub",
        "error": "cloud_call_not_implemented",
        "provider_hint": PROVIDER_HINT,
        "read_only": READ_ONLY,
        "env": ENV_VAR,
        "note": "real SDK call pending cloud account provisioning",
    }
