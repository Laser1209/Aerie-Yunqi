"""douyin-interactive-publish skill (cloud) — 抖音互动空间发布 / Interact publish.

Block-5B scaffold. Cloud-based skill that requires external
credentials (env: `DOUYIN_OPEN_ID`). When the env var is
missing we return a stub so the main pipeline is never broken.
The actual HTTP/sign/SDK call is intentionally a TODO until
the corresponding cloud account is provisioned.
"""
from __future__ import annotations
import logging
import os
logger = logging.getLogger(__name__)

PROVIDER_HINT = "text"
READ_ONLY = False
ENV_VAR = 'DOUYIN_OPEN_ID'


def run(args: dict) -> dict:
    """Skill entry point. ``args`` is a free-form dict; key: 'zip_path'."""
    args = args or {}
    zip_path_value = args.get("zip_path")
    if not zip_path_value:
        return {"error": "missing zip_path", "provider_hint": PROVIDER_HINT}

    if not os.getenv('DOUYIN_OPEN_ID'):
        return {
            "status": "stub",
            "error": f"credential_missing: env {ENV_VAR!r} not set",
            "provider_hint": PROVIDER_HINT,
            "read_only": READ_ONLY,
            "zip_path": str(zip_path_value)[:80],
        }

    # TODO: actual cloud call
    return {
        "status": "stub",
        "error": "cloud_call_not_implemented",
        "provider_hint": PROVIDER_HINT,
        "read_only": READ_ONLY,
        "env": ENV_VAR,
        "note": "real SDK call pending cloud account provisioning",
    }
