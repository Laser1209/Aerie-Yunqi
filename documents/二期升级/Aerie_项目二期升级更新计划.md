...
| 当前执行状态 | `P2-00-B01`、`P2-00-B02`、`P2-00-B04`、`P2-00-B06`、`P2-00-B07` Done；`P2-00-B03` Review；`P2-00-B05` Blocked/未授权；56 个顶层控制行已完成追踪映射，4 项二期候选 ADR 待 PO 批准；`B02-BLK-002` 与 `B02-BLK-006` 已关闭，B07 已收口 `B02-BLK-005` 的仓库根路径子项并将离线 E2E 推进到 `18/21`，但该聚合阻断与其余 5 项仍保持 Open；G0 Open，尚未进入 `P2-01` 及 G0 后功能批次 |
...
治理文档按批次创建，不提前创建无证据的空文件。当前状态如下：

| 文件 | 当前状态 | 用途 |
|---|---|---|
| `00_二期基线与环境冻结.md` | Done | P2-00-B01 源基线、工作区归属与隔离边界 |
| `01_二期需求追踪矩阵.md` | Review | P2-00-B03 目标、缺口、一期、当前实现、批次、门禁与 Evidence 双向追踪 |
| `02_二期架构决策记录.md` | Review | P2-00-B03 继承 ADR、二期候选 ADR、所有权与共享合同串行规则 |
| `03_二期风险登记册.md` | Review | P2-00-B03 风险责任、阈值、控制、应急、回滚与 B02 阻断映射 |
| `04_二期测试与证据索引.md` | Done with residual blockers | P2-00-B02 基线证据、P2-00-B04 扫描门、P2-00-B06 QQ 禁用路径、P2-00-B07 离线 E2E 路径修复证据及剩余阻断台账 |
| `05_二期部署与回滚记录.md` | Not created | 后续部署与回滚批次有真实证据时创建 |
| `06_二期发布验收清单.md` | Not created | P2-12 发布候选具备真实产物时创建 |
| `07_二期任务编排.md` | Active | 二期批次、依赖、门禁、状态和 Git 单元边界 |
