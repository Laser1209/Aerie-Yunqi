...
> [!note] QQ 桥接是可选能力
> NapCat 只用于 QQ 收发消息。==不装 NapCat 也能正常使用==——桌面聊天、生图、世界模拟、办公、记忆、后台管理都不受影响；主动消息会自动改走桌面端（气泡 + 系统通知），生图会自动切到本地聊天。启动时检测不到 QQ 会等最多 30 秒后自动进入「降级模式」照常运行，不会卡住或崩溃。
...
| 没装 NapCat / QQ 能用吗 | 能。QQ 是可选能力，桌面聊天、生图、世界模拟、办公、记忆都不受影响；主动消息会自动改走桌面端 |
| 界面显示异常 / 白屏 | 重启软件试试 |
