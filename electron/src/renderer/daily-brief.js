"use strict";
/* Aerie · 云栖 v9.0 — Daily Brief popup controller (Block-4A R1.3).
 *
 * 在 iframe (electron/src/renderer/index.html 内嵌) 或独立窗口中运行。
 * - init(): GET /api/brief/today → 渲染 5 section
 * - render(json): 填 5 个 list + weather
...
class DailyBrief {
  ...
  _notifyMain(channel) {
    if (window.aerie && window.aerie.electron && window.aerie.electron.notify) {
      window.aerie.electron.notify(channel);
    }
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.__briefInstance = new DailyBrief();
});
