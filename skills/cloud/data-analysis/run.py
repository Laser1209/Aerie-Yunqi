"""data-analysis skill (cloud) — Excel/CSV 数据分析 / Data analysis.
...
def run(args: dict) -> dict:
    """Skill entry point. ``args`` is a free-form dict; key: 'xlsx_path'."""
    args = args or {}
    xlsx_path_value = args.get("xlsx_path")
    if not xlsx_path_value:
        return {"error": "missing xlsx_path", "provider_hint": PROVIDER_HINT}
    ...
    if not True:  # 无需凭据
        return {
            ...
            "read_only": READ_ONLY,
            "xlsx_path": str(xlsx_path_value)[:80],
        }
