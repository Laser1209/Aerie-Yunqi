...
| # | 文件 | 改动 |
|---|---|---|
| 1 | [core/conversation_continuity.py](file:///e:/Agent_reply/core/conversation_continuity.py) | ContextAssembler：`recent_turn_limit=8` 轮次化分组；分组摘要注入；system prompt 通道段；历史来源标记 |
| 2 | [core/conversation_repository.py](file:///e:/Agent_reply/core/conversation_repository.py) | history_page 支持按 turn 分组返回；摘要桶查询 |
| 3 | [core/companion.py](file:///e:/Agent_reply/core/companion.py#L487-L492) | ContextAssembler 参数改为 `recent_turn_limit=8, max_summary_buckets=3` |
...
| 5 | [memory/layers/layered_memory.py](file:///e:/Agent_reply/memory/layers/layered_memory.py) | store 支持 channel/source metadata；search 结果透出来源 |
| 6 | [memory/layers/sync_adapter.py](file:///e:/Agent_reply/memory/layers/sync_adapter.py) | retrieve 返回 channel 字段 |
| 7 | [core/pipeline.py](file:///e:/Agent_reply/core/pipeline.py) | 摘要分桶调度；记忆来源注入；`_default_rolling_summary` 支持分桶 |
| 8 | [config/settings.yaml](file:///e:/Agent_reply/config/settings.yaml) | feature_flags 增加 `thinking_trace_injection_v1: false` |
| 9 | 新增测试 | 轮次窗口、分组摘要、通道感知注入、跨通道隔离、记忆来源 |
...
| 风险 | 应对 |
|---|---|
| 轮次窗口 8 轮增大 token | 已在 24k 预算内；若超限，`max_total_chars` 裁温层/冷层，热窗口优先 |
| 彻底隔离下跨端接话仍失败 | 方案 v2 开启"跨通道共享摘要桶"（3.5 可选增强），或热窗口补跨通道最近 1 轮上下文 |
| 分组摘要表迁移影响现有 summary | 新表独立建，旧 `conversation_summaries` 保留只读兼容，不回滚 |
| 记忆来源标注改动影响检索兼容 | 仅在 metadata 增加字段，缺失时默认 `channel=unknown` 不破坏现有查询 |
| 思维链注入 token 膨胀 | 默认关闭 flag，按需开启 |

---

## 7. 实施顺序

1. **P0（最小可用）**：热窗口轮次化（3.1）+ 通道感知注入（3.4）→ 解决"跨通道断链 + 不知道自己在哪端"两个直接故障
2. **P1**：分组摘要（3.2）+ 记忆来源标注（3.3/3.7-1）
3. **P2（可选）**：思维链 trace + 自省注入（3.6）、写入校验门（3.7-2）、跨通道共享摘要桶（3.5-3）
