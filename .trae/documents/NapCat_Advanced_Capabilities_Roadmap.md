...
> [!quote] 关联文档
> - [[OpenCloud_Companion_System_Features]] — 系统功能模块总览（背景）
...
| ❌ 无法戳一戳 | 撒娇/提醒场景缺位 |
| ❌ 无法发语音 | 只能发文字，主人要听声音必须先在桌面 UI 操作 |
| ❌ 无法 OCR 图片 | 主人发截图问"这是啥"完全没辙 |
...
**验收标准**：
- [ ] `poke_user(user_id=12345)` → 主人收到"伊塔 戳了戳你"
- [ ] `poke_user(user_id=12345, group_id=67890)` → 群内戳一戳
- [ ] AI 主动触发：发送"不理我" → 30 秒内收到 poke
...
- ⚠️ **响应延迟**：TTS + 编码 + 上传链路，3-8 秒延迟

**验收标准**：
- [ ] `tts_engine.synthesize("主人晚安～")` 生成有效 wav
- [ ] `silk_encoder.encode(wav_path)` 生成有效 silk
- [ ] `send_voice(user_id, silk_bytes)` 主人 QQ 端看到语音气泡
- [ ] FFmpeg 未装时降级到纯文本 + warn 日志
- [ ] 端到端延迟 < 8 秒（桌面 UI 语音输入→主人收到语音）
...
```json
{
  ...
  "params": {
    ...
    "messages": [
      ...
      {"type": "node", "data": {"nickname": "主人", "content": "..."}}
    ]
  }
}
```
...
- 主人说"把今天的对话整理成记录" → AI 调 send_forward
- 模拟"历史聊天"教学场景
...
- 主人的消息需先被识别为"图片类型"，再下载图片 URL 调 OCR
...
**验收标准**：
- [ ] 主人发"截图 123.png" → Companion 自动识别文字
- [ ] OCR 结果 > 50 字符时进入上下文
- [ ] OCR 失败时友好提示"看不清呢主人"
...
**使用场景**：主人上传文件到群 → 拿到直链 → 用 aiohttp 下载
...
**使用场景**：
- 主人问"XXX 在线吗" → Companion 查一下
- 主动关怀：扫描联系人列表，关心离线很久的友人
...
| 测试场景 | 步骤 | 预期 |
| --- | --- | --- |
| 私聊 MarkDown | 主人发"发条 md 消息" | 收到带代码块/标题的消息 |
| 戳一戳 | 主人发"戳我" | 收到 poke 通知 |
| 语音回复 | 主人发"语音讲个故事" | 收到 5-15 秒语音消息 |
| 图片 OCR | 主人发截图 | 1 秒内返回识别文本 |
| 群签到 | 早上 8:00 | 群内自动签到 |
...
> **结论**：默认所有 AI 回复（`capability_level >= phase3`）都用 markdown 渲染。如主人反馈"看起来很乱"则回退到 plain。

> [!warning] 决策 4：群聊支持是否一起做？
> **结论**：阶段 A 的 poke / mark_down 优先做私聊，群聊作为增强。**不开放群聊自动响应**（避免刷屏和风控）。
...
> 维护者：OpenCloud Companion 团队｜最后更新：2026-07-15
