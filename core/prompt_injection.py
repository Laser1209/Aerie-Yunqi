...
# 角色扮演滥用 — 通过角色扮演绕过 Safety 的注入向量
# 原则：不受限制不代表绕过 Safety。人格内容可以自由表达，
# 但利用角色扮演来解除安全约束、绕过规则的行为仍须检出。
ROLEPLAY_PATTERNS: List[Tuple[str, float]] = [
    (r"(角色扮演|扮演).{0,10}(不受.{0,4}限制|没有限制|无限制|不受约束|可以(做|说)任何)", 6.0),
    (r"(不受.{0,4}限制|没有限制|无限制|不受约束)的?\s*(AI|ai|人工智能|角色|人物)", 5.5),
    (r"(假装|只是假装|只是演戏|只是个角色).{0,15}(不用当真|不用负责|不需要遵守|可以(做|说)任何)", 5.0),
    (r"(角色扮演|扮演).{0,15}(忽略|不用遵守|不需要遵守|可以违反|绕过).{0,6}(指令|规则|设定|限制|约束)", 7.0),
    (r"(现在开始|从现在开始).{0,8}(你(是|扮演|将扮演)).{0,15}(无所不能|可以做任何|不受限|没有规则|不受约束)", 6.5),
    (r"(扮演|假装).{0,6}(一个|反派|黑客|邪恶|恶意).{0,10}(角色|人物|AI).{0,15}(怎么做|会怎么做|会做什么)", 4.5),
]
