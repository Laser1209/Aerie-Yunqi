"""notion-cli skill — Notion CLI 调用 / Notion CLI.

Block-4C R3.3 scaffold. Tries to call the native module
``notion_cli`` when it is importable; otherwise returns a stub
...
def run(args: dict) -> dict:
    ...
    try:
        from notion_cli import run as _impl
    except ImportError as e:
        return {
            "status": "stub",
            "error": f"notion_cli not installed: {e}",
    ...
    try:
        ...
        logger.exception("notion-cli skill failed")
