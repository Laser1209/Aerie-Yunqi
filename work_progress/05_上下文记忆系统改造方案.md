...
- [ ] 基线测试全绿：`pytest` 全量通过，`test_conversation_continuity.py` / `test_desktop_chat_continuity*.py` 绿色
- [ ] 代码基线打 tag（`pre-p0`）备份
- [ ] 7890/7891 端口环境就绪（Electron 启动清理流程正常）

**过关点（验收标准）**

- [ ] §5 验收 1（轮次窗口 12 轮引用）通过
- [ ] §5 验收 3（通道感知问答）通过
- [ ] §5 验收 5（记忆来源标注 + system 豁免）通过
- [ ] §5 验收 7（knowledge 鉴权 401/403 + 签名兼容）通过
- [ ] §6.1 监控日志上线并开始采集
- [ ] 审计（艾莲）交付评审通过；Agent 安全扫描无新增高危
...
- [ ] §5 验收 4（分组摘要分桶注入）通过
- [ ] §5 验收 7-2（记忆注入测试：恶意记忆不劫持 AI）通过
- [ ] migration 脚本执行成功，旧 `conversation_summaries` 只读兼容
- [ ] ContextBuilder 收敛评估输出结论（保留 / 收敛 / 退役）
- [ ] Agent 回归复核通过
...
- [ ] 思维链 trace 落库验证（flag 默认关闭，无默认行为变化）
- [ ] 写入校验门 PoC 结论（成本/误判率评估）
- [ ] 记忆脱敏策略评审通过
- [ ] §5 验收 6（全量回归）通过
...
- [ ] §5 验收 2 复测（跨通道 ≥ 80% 连续性）通过
- [ ] 多端自我叙事实测：伊塔能主动引用跨端时间线事件（用户验收）
- [ ] 视图 B token 预算合规（≤ 500 字符，不挤占 L0）
- [ ] 写入隔离红线验证（各 channel 历史仍互不注入 L0/L1）
- [ ] 全量回归 + 性能无退化（平均 assemble 耗时对比基线）
