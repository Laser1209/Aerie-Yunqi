"""douyinpay-payment skill (cloud) — 抖音支付 / DouyinPay.

Block-5B scaffold. Cloud-based skill that requires external
credentials (env: `DOUYINPAY_MCH_ID`). When the env var is
...
def run(args: dict) -> dict:
    """Skill entry point. ``args`` is a free-form dict; key: 'out_order_no'."""
    args = args or {}
    out_order_no_value = args.get("out_order_no")
    if not out_order_no_value:
        return {"error": "missing out_order_no", "provider_hint": PROVIDER_HINT}

    if not os.getenv('DOUYINPAY_MCH_ID'):
        return {
            "status": "stub",
            "error": f"credential_missing: env {ENV_VAR!r} not set",
            "provider_hint": PROVIDER_HINT,
            "read_only": READ_ONLY,
            "out_order_no": str(out_order_no_value)[:80],
        }
