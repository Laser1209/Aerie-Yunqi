...
> [!abstract] 摘要（v2.0 · 升级后修订）
> 本文档针对 Aerie · 云栖 桌面应用 **3-6 分钟异常启动加载时间** 进行根因诊断与分级优化。**v2.0 基于 2026-08-12 系统升级后的最新架构、技术栈与实测性能指标全面修订**。核心结论：
> - **升级后实测基线**（2026-08-12 21:23）：import 阶段 **10.93 秒**、全量测试 **1278 passed / 27 failed**（与基线一致，无新增回归）。
> - **根因诊断**：启动慢由 **10 个串行阻塞点叠加**导致，单一最大瓶颈为 `await self.qq.wait_until_ready(timeout=30.0)`（[[#4.2 单一最大瓶颈：QQ 30 秒同步等待|见 §4.2]]）。
> - **升级后新增架构**：`world_inprocess_v1`/`world_image_candidates_v1`/`multi_channel_identity_v1` 等开关开启、B08 architecture_guard 架构治理工具、brain→llm_caller 命名收敛、InProcess 世界保护修复（[[#2.3 升级后架构变更|见 §2.3]]）。
> - **可行性评估**：保守可达 30-60s（3-6x），进阶可达 10-20s，**10s 目标在技术上确认可达**（[[#7. 10 秒目标可行性评估|见 §7]]）。
> - **头号收益项**：QQ 等待改后台非阻塞（-30s）+ asyncio.gather 并行化（-10~-30s）+ 附件模块 import 懒加载（-4.4s）。
>
> 相关文档：[[01_模块总览]]、[[09_当前状态]]、[[02_技术总览]]、[[10_模块依赖关系图]]、[[boot_trace_data]]、[[plans/Aerie-Model-X分支升级执行计划]]

---

## 0. 升级后现状总览（2026-08-12）

> [!info] v2.0 修订背景
> 本版基于 **main ↔ Aerie-Model-X 双向同步完成后的最新系统状态** 修订。升级过程见 [[plans/Aerie-Model-X分支升级执行计划]]，主要变更：
>
> | 变更项 | 类型 | 状态 |
> |---|---|---|
> | main 126 个提交 + 工作区修复同步至 Aerie-Model-X | 分支同步 | ✅ 完成（merge `7c544b7`） |
> | B08 architecture_guard 架构治理工具（4 规则，204 文件） | 新功能 | ✅ 已入 main（`41a66af`） |
> | brain → llm_caller 命名收敛修复 | 重构 | ✅ 已入 main（`14cf306`） |
> | world_runtime_bind InProcess 世界保护（修复生图链路） | Bug 修复 | ✅ 已入 main（`e69093d`） |
> | brief_greeting_fresh 实时读 DB todo | Bug 修复 | ✅ 已入 main（`e69093d`） |
> | multi_channel_identity_v1 开启 | 功能开关 | ✅ 已入 main（`e69093d`） |
> | 全量回归 1278 passed / 27 failed | 验证 | ✅ 与基线一致，无新增回归 |
> | import 阶段性能实测 | 测量 | ✅ 10.93 秒（见 [[boot_trace_data]]） |

### 0.1 升级后启动相关配置（config/settings.yaml）

| feature_flag | 值 | 启动性能影响 |
|---|---|---|
| `world_inprocess_v1` | **true** | 进程内世界模拟器启动，增加初始化负担 |
| `world_image_candidates_v1` | **true** | 生图链路启用（依赖 InProcess 世界保护修复） |
| `multi_channel_identity_v1` | **true** | 多通道身份（同步时开启） |
| `context_budget_v1` / `conversation_model_v1` | true | 上下文预算，影响首条消息响应 |
| `proactive_delivery_v2` | true | 主动消息（依赖 QQ 就绪） |
| `world_sidecar_v1` / `mobile_gateway_v1` | false | 未启用，减少启动负担 |

> [!warning] 关键启示
> 升级后 `world_inprocess_v1: true` 意味着启动时需初始化进程内世界模拟器；`qq.startup_wait_timeout` 仍为 **30 秒**（见 [core/companion.py:849](file:///e:/Agent_reply/core/companion.py#L849)），与 §4.2 诊断一致——**优化尚未实施，本文档方案仍具时效性**。

### 0.2 升级后 import 实测对比（两次测量）

| 指标 | v1.1 基线（2026-08-11 17:55） | v2.0 升级后（2026-08-12 21:23） | 变化 |
|---|---|---|---|
| import 总耗时 | 7.89s | **10.93s** | +3.04s（代码量增长） |
| Import 条目数 | 1653 | 1658 | +5 |
| numpy/pandas self | 2733ms | **4149ms** | +1416ms |
| fastapi/uvicorn self | 777ms | **1458ms** | +681ms |
| 最重链 core.companion | 4989ms | **7188ms** | +2199ms |
| 完整数据 | [[boot_trace_data]] | [[boot_trace_data]] | 详见 §4.3 |

> [!danger] 升级后 import 耗时反而上升
> 系统升级（新增 B08、多通道、世界图片候选等）使 import 阶段从 7.89s 升至 **10.93s**，已超过 Nielsen 10 秒注意力极限。**懒加载优化（U-7/U-8）紧迫性上升**。
...
初版分析遵循用户要求 **"在保持代码完整性不做任何修改的前提下"**，所有结论均基于只读代码分析、项目记忆、互联网公开数据交叉验证得出。**v2.0 补入升级后的实际运行数据（性能实测、测试结果）进行交叉验证**。
...
| 变更 | 影响位置 | 对启动性能的影响 |
|---|---|---|
| B08 architecture_guard | `tools/architecture_guard.py`（204 文件扫描） | 独立工具，**不进启动链**，仅 CI/审计使用 |
| brain → llm_caller 命名收敛 | `core/llm_caller.py`（原 brain.py） | import 链不变，无性能影响 |
| world_runtime_bind InProcess 保护 | `core/api_server.py` | 修复运行时生图链路，**不影响启动** |
| brief 实时读 todo | `core/api_server.py` | 启动后首条问候语变慢 ~20ms，**不阻塞启动** |
| multi_channel_identity_v1 | settings.yaml | 启动时多通道身份初始化，**微增 import/init** |
...
| L10g | `core/companion.py:623-644` | `Pipeline(...)` 完整管道构造 | 同步 | <50ms | ✅ 是 |
...
| L11e | `core/companion.py:798` | `asyncio.create_task(qq.connect())` 后台 | 异步后台 | <10ms | 否 |
| L11f | `core/companion.py:581-587` | 5 个 `create_task` 后台循环 | 异步后台 | <10ms | 否 |
| L11g | `core/companion.py:819` | **`await _seed_identity_memories()`** 3 条种子 × (search+store) | 异步阻塞 | **1-18s** | ✅ 是 ⚠️ |
...
| L11k | `core/companion.py:855` | **`await qq.wait_until_ready(timeout=30.0)`** | 异步阻塞 | **0-30s** | ✅ 是 ⚠️⚠️ |
...
> **升级后附加**：import 阶段实测 **10.93s**（v1.1 时 7.89s），见 [[#4.3 importtime 实测验证（2026-08-12 升级后）|§4.3]]
...
> - [core/companion.py:819](file:///e:/Agent_reply/core/companion.py#L819) `await self._seed_identity_memories()` 阻塞主流程
> - [core/companion.py:855](file:///e:/Agent_reply/core/companion.py#L855) `await self.qq.wait_until_ready(timeout=30.0)` 同步等待 30 秒
...
> - [core/companion.py:2322-2360](file:///e:/Agent_reply/core/companion.py#L2322-L2360) `_seed_identity_memories` 循环 3 条种子，每条先 search 检查存在性再 store = 6 次向量操作
>
...
| B: _seed_identity_memories 是大头 | 该函数有 try/except 全包裹，失败会降级 | [core/companion.py:2322](file:///e:/Agent_reply/core/companion.py#L2322) 整个函数被 try/except 包裹，但 **内部 await 仍会阻塞主流程** | 6-18s（远程）/ 3-10s（本地 ONNX） |
...
> [!success] 根因诊断最终结论（v2.0 复核仍成立）
> **Aerie 启动慢的根因是 10 个串行阻塞点叠加**，按耗时贡献排序：
>
> | 排名 | 阻塞点 | 文件位置 | 典型耗时 | 占比 |
> |---|---|---|---|---|
> | 🥇 #1 | `await qq.wait_until_ready(timeout=30.0)` | [companion.py:855](file:///e:/Agent_reply/core/companion.py#L855) | 5-30s | 30-50% |
> | 🥈 #2 | `await _seed_identity_memories()` 6 次向量操作 | [companion.py:819](file:///e:/Agent_reply/core/companion.py#L819) | 6-18s | 15-25% |
> | 🥉 #3 | **import 阶段本身 10.93s**（升级后） | 全模块 | 10.9s | 10-15% |
> | #4 | `LayeredMemory` + `resolve_embedding_fn()` 构造 | [companion.py:309-313](file:///e:/Agent_reply/core/companion.py#L309-L313) | 3-10s | 10-15% |
> | #5 | **numpy/pandas import 4.15s**（升级后） | 附件处理链 | 4.1s | 5-10% |
> | #6 | **markitdown import 5.97s**（升级后） | 附件处理链 | 6.0s | 5-10% |
> | #7 | Electron `_evictOrphanBackendIfNeeded` netstat+wmic | [main.js:359-414](file:///e:/Agent_reply/electron/src/main.js#L359-L414) | 2-5s | 5-10% |
> | #8 | `SkillLoader.discover() + register_all()` 17 技能 | [companion.py:607-608](file:///e:/Agent_reply/core/companion.py#L607-L608) | 200-1000ms | 1-3% |
> | #9 | `seed_dialogue(knowledge)` 同步知识播种 | [companion.py:558-561](file:///e:/Agent_reply/core/companion.py#L558-L561) | 100-500ms | <1% |
> | #10 | Windows Defender 扫描放大（隐形） | 系统层 | 不可预测，可达 14s+ | 不可量化 |
>
> **核心问题不是单一组件慢，而是串行 await 链导致耗时累加**。升级后 import 阶段的 3 个隐形大头（import 总 10.9s / numpy 4.1s / markitdown 6.0s）合计占比上升，懒加载优化的优先级上调。

### 4.2 单一最大瓶颈：QQ 30 秒同步等待

> [!danger] 头号根因
> [core/companion.py:855](file:///e:/Agent_reply/core/companion.py#L855) 的 `await self.qq.wait_until_ready(timeout=wait_timeout)` 是启动链中**单一最大阻塞点**。
>
> ```python
> # core/companion.py:849-855（升级后行号，逻辑未变）
...
> ```
>
> **问题本质**：QQ 登录握手依赖 Tencent 服务器，正常场景 5-15 秒，异常场景 30 秒超时。这段时间内 **整个后端启动流程被冻结**，无法启动 uvicorn HTTP 服务，导致 Electron 健康检查永远命中失败。升级后 `startup_wait_timeout` 仍为 30.0（见 [config/settings.yaml](file:///e:/Agent_reply/config/settings.yaml)），**该瓶颈未缓解**。

### 4.3 importtime 实测验证（2026-08-12 升级后）

> [!info] 实测说明
> 使用 `python -X importtime` 对 **升级后 main/Aerie-Model-X 合并分支** 的 import 阶段进行实测。
...
| 指标 | 数值 | 与 v1.1 基线对比 |
|---|---|---|
| **Probe 总耗时** | **10.93 秒** | +3.04s（代码量增长） |
| Import 条目数 | 1658 | +5 |
| 所有模块 self 累计 | 10557 ms ≈ 10.56 s | +3046ms |
| 最重导入链 | `core.companion` (7188 ms cumulative) | +2199ms |
| 自身耗时最大模块 | `numpy._core._multiarray_umath` (1117 ms self) | +254ms |

> [!danger] 关键发现：升级后 import 阶段从 7.89s 升至 10.93s
> 系统升级（新增 B08 architecture_guard、多通道身份、世界图片候选等）使 import 阶段超过 Nielsen 10 秒注意力极限。**这是启动优化必须优先解决的硬底线**。

#### 4.3.2 重型依赖分类汇总（升级后）

| 类别 | self 累计 (ms) | 模块数 | 占 self 总比 | 与 v1.1 对比 | 诊断 |
|---|---|---|---|---|---|
| **numpy/pandas** | **4149** | 400 | **39.3%** | +1416ms | 🔴 头号 import 瓶颈，仅附件处理用到 |
| fastapi/uvicorn | 1458 | 81 | 13.8% | +681ms | ⚠️ 必需，但可延迟路由注册 |
| aerie_internal | 680 | 104 | 6.4% | +106ms | ⚠️ 项目自身模块 |
| pydantic | 233 | 68 | 2.2% | +1ms | FastAPI 依赖 |
| onnxruntime | 232 | 10 | 2.2% | +50ms | ✅ ONNX 模型加载不是 import 问题 |
| httpx/requests | 104 | 42 | 1.0% | +33ms | ✅ 正常 |
| yaml/toml/dotenv | 85 | 22 | 0.8% | +35ms | ✅ 正常 |
| **未出现** | — | — | — | — | chromadb 未进入 Top 30，其 import 被懒加载或在函数内 |

#### 4.3.3 Top 10 慢导入（按累计耗时 cumulative，升级后）

| # | 累计 (ms) | 自身 (ms) | 模块 | 诊断 |
|---|---|---|---|---|
| 1 | **7188** | 16.7 | `core.companion` | 🔴 触发最深递归 import 链 |
| 2 | 6335 | 6.5 | `core.pipeline` | 🔴 Pipeline 模块连带加载 |
| 3 | 6321 | 257 | `core.attachment_handler` | 🔴 附件处理器连带 markitdown |
| 4 | **5968** | 4.8 | `markitdown` | 🔴 隐藏瓶颈！文档转换库 |
| 5 | 5960 | 3.6 | `markitdown._markitdown` | 🔴 markitdown 内部 |
| 6 | 4025 | 6.5 | `markitdown.converters` | markitdown 转换器 |
| 7 | 3381 | 2.6 | `markitdown.converters._xlsx_converter` | XLSX 转换（连带 pandas） |
| 8 | **3373** | 8.7 | `pandas` | 🔴 数据分析库，仅附件处理用到 |
| 9 | 3020 | 155 | `core.api_server` | API 服务器模块 |
| 10 | 1696 | 21 | `magika` | 文件类型检测（AI 模型） |

#### 4.3.4 Top 10 自身耗时最重的模块（按 self time，升级后）

| # | 自身 (ms) | 累计 (ms) | 模块 | 优化建议 |
|---|---|---|---|---|
| 1 | **1117** | 1126 | `numpy._core._multiarray_umath` | 🔴 numpy 核心扩展，改函数内 import |
| 2 | **1001** | 1213 | `fastapi.exceptions` | ⚠️ FastAPI 异常定义，必需 |
| 3 | 294 | 317 | `pyarrow.lib` | 🔴 Apache Arrow，pandas 依赖 |
| 4 | 257 | 6321 | `core.attachment_handler` | 🔴 附件处理器，连带 markitdown |
| 5 | 155 | 3020 | `core.api_server` | ⚠️ 项目自身，可拆分路由懒加载 |
| 6 | 133 | 166 | `fastapi.openapi.models` | ⚠️ OpenAPI 模型定义 |
| 7 | 130 | 130 | `onnxruntime.capi.onnxruntime_pybind11_state` | ✅ ONNX C++ 扩展，比预期快 |
| 8 | 105 | 105 | `main` | 主入口模块 |
| 9 | 99 | 165 | `weasyprint.text.ffi` | 🔴 PDF 生成库 FFI 绑定，仅导出 PDF 时用到 |
| 10 | 82 | 220 | `onnxruntime.capi._pybind_state` | ONNX pybind 绑定 |

#### 4.3.5 升级后根因排序修正

| 排名 | v1.1 假设 | v2.0 升级后修正 | 依据 |
|---|---|---|---|
| 🥇 #1 | QQ 30s 等待 | **维持**（运行时未测，但代码逻辑明确，未缓解） | [companion.py:855](file:///e:/Agent_reply/core/companion.py#L855) |
| 🥈 #2 | _seed_identity_memories 6-18s | **维持**（运行时未测） | [companion.py:819](file:///e:/Agent_reply/core/companion.py#L819) |
| 🥉 #3 | import 阶段 7.89s | **上调**：升至 **10.93s**，已超 Nielsen 极限 | 实测 probe=10.93s |
| #4 | LayeredMemory + ONNX 3-10s | **维持**：ONNX import 仅 232ms，瓶颈在运行时模型加载 | 实测 onnxruntime self=232ms |
| #5 | numpy/pandas import 2.73s | **上调**：升至 **4.15s**（39.3%） | 实测 numpy/pandas self=4149ms |
| #6 | markitdown import 4.14s | **上调**：升至 **5.97s** | 实测 markitdown cum=5968ms |
...
> [!warning] 升级后根因排序（10 个串行阻塞点）
> 升级后 import 阶段的 3 个隐形大头合计占比上升至 **30%+**（import 总 10.9s + numpy/pandas 4.1s + markitdown 6.0s）。这意味着即使完美并行化所有运行时任务，**import 阶段的 10.93 秒仍是硬底线**，必须通过懒加载来消除。

#### 4.3.6 升级后新增优化靶点

| 新靶点 | 文件位置 | 实测耗时（升级后） | 优化方案 | 预期收益 |
|---|---|---|---|---|
| `markitdown` 文档转换库 | [core/attachment_handler.py](file:///e:/Agent_reply/core/attachment_handler.py) import 链 | 5968 ms cumulative | 改为函数内 import，仅处理附件时加载 | **-6.0s** |
| `numpy` + `pandas` 科学计算栈 | markitdown → pandas → numpy 连带 | 4149 ms self（39.3%） | 改为函数内 import，仅附件分析时加载 | **-4.1s** |
| `fastapi.exceptions` 异常定义 | [core/api_server.py](file:///e:/Agent_reply/core/api_server.py) import 链 | 1001 ms self | 拆分路由懒加载 | **-1.0s** |
| `weasyprint` PDF 生成库 | api_server import 链 | 99 ms self + FFI 绑定 | 改为函数内 import，仅导出 PDF 时加载 | **-0.2s** |
| `magika` 文件类型检测 | markitdown → magika 连带 | 1696 ms cumulative | 改为函数内 import | **-1.7s**（含 magika 自身） |

> [!tip] 附件处理模块是 import 阶段的最大浪费（升级后加剧）
> `core.attachment_handler` → `markitdown` → `pandas` → `numpy` → `pyarrow` → `magika` 这条 import 链合计 **~6.4 秒**（升级后），而这些库 **只在用户上传附件时才用到**。改为函数内 import 可直接节省 6 秒+，是 ROI 最高的 import 优化。

#### 4.3.7 升级后启动耗时数学模型

| 阶段 | v1.1 估算 | v2.0 实测修正 | 依据 |
|---|---|---|---|
| Electron 启动 + Python spawn | 5s | 5s | 维持 |
| **import 阶段** | 7.89s | **10.93s** | 🔴 实测（升级后） |
| Companion.__init__（含 ChromaDB） | 3-10s | 3-10s | 维持（ONNX import 仅 232ms） |
| companion.start() 串行 await | 35-150s | 35-150s | 维持（QQ + _seed_identity） |
| API 启动 + 移动网关 | 2s | 2s | 维持 |
| Windows Defender 放大 | 5s | 5s | 维持 |
| **总计** | **58-180s + 7.89s** | **58-180s + 10.93s** | — |

> [!success] 修正后的 10s 目标可行性
> 升级后实测让 10s 目标**更加紧迫但也更可行**：
> - import 阶段 10.93s 中有 **~6.4s 可通过附件模块懒加载直接消除**
> - 剩余 import ~4.5s（fastapi/aerie_internal/pydantic）是必需成本
> - 配合运行时并行化（U-1/U-2）+ 懒加载（U-3）+ import 懒加载（U-7/U-8）
> - **理论可达：4.5s（必需 import）+ 2s（Electron+spawn）+ 3s（并行化后的运行时初始化）≈ 9.5s**
> - **10s 目标在技术上确认可达，但依赖 U-7 附件懒加载的实施**
...
|---|---|---|---|---|
| `_seed_identity_memories` | [companion.py:2322-2360](file:///e:/Agent_reply/core/companion.py#L2322-L2360) | `companion.start()` 中 `await` | 3 条种子 × (search+store) = 6 次向量操作 | ✅ 阻塞 |
...
| `world_inprocess_v1` 初始化 | `core/world_port.py` | `companion.start()` 后台 | InProcess 世界模拟器启动 | ⚠️ 升级后新增 |
...
> `sync_persona_yaml_to_hub.py` 是独立脚本，需手动运行。**启动时不触发**，不构成瓶颈。升级后 `multi_channel_identity_v1` 开启，身份解析走多通道逻辑（见 `core/identity_resolver.py`）。
...
> [!success] 上策目标（v2.0 调整）
> **改动范围**：核心启动序列重构
> **理论提速**：3-6min → **12-20s**（12-30x）
> **风险等级**：中（需充分测试）
> **实施难度**：高
> **状态**：**均未实施**（升级后复核确认代码仍在串行 await）

#### 6.1.1 U-1：QQ 等待改后台非阻塞（头号收益项）

**当前问题**：[companion.py:855](file:///e:/Agent_reply/core/companion.py#L855) `await qq.wait_until_ready(timeout=30.0)` 阻塞整个启动链 30 秒。升级后 `startup_wait_timeout` 仍为 30.0。
...
**当前问题**：[companion.py:819-855](file:///e:/Agent_reply/core/companion.py#L819-L855) 串行 await 5 个互不依赖的任务。
...
**当前问题**：[api_server.py:139-149](file:///e:/Agent_reply/core/api_server.py#L139-L149) 使用已废弃的 `@app.on_event("startup"/"shutdown")`（实测警告确认）；同步阻塞操作可能冻结事件循环。
...
**预期收益**：**-1 到 -3 秒** + 事件循环稳定性提升 + 消除废弃 API 警告
...
#### 6.1.7 U-7：附件处理模块 import 懒加载（升级后加重）

**当前问题**：[core/attachment_handler.py](file:///e:/Agent_reply/core/attachment_handler.py) 在模块顶层 import `markitdown`，连带加载 `pandas`/`numpy`/`pyarrow`/`magika`。**升级后实测合计 6.4 秒**（v1.1 时 4.4s）。这些库只在用户上传附件时才用到。

**实测依据**：[[#4.3 importtime 实测验证（2026-08-12 升级后）|§4.3 实测]]

**改造方案**：

```python
# core/attachment_handler.py 当前（顶层 import）
import markitdown  # 连带 pandas/numpy/pyarrow/magika，6.4s
...
**预期收益**：**-6.4 秒**（升级后实测，直接消除 import 阶段最大浪费）

#### 6.1.8 U-8：core.companion 递归 import 链瘦身（升级后加重）

**当前问题**：`core.companion` cumulative 7188ms，是 import 阶段最重的递归链。其中连带加载了 `core.pipeline`(6335ms)、`core.attachment_handler`(6321ms) 等模块。

**改造方案**：将 `core.companion` 中对非必需模块的 import 改为函数内 import：

```python
# core/companion.py 当前
from core.pipeline import Pipeline  # 连带 6.3s
from core.attachment_handler import AttachmentHandler  # 连带 6.3s
...
**预期收益**：配合 U-7，**-2 到 -4 秒**（消除 core.companion → core.pipeline/attachment_handler 的连带加载）
...
| **U-7 附件模块 import 懒加载** | **-6.4s** | **-54.4~-97.4s** |
| **U-8 companion import 链瘦身** | **-2~-4s** | **-56.4~-101.4s** |
| **典型场景** | 3-6min → **10-15s** | 18-36x |
...
识别耗时 Top 10 模块（升级后实测：numpy 1117ms、fastapi.exceptions 1001ms、pyarrow 294ms 等），将非启动必需的重依赖（如 `pandas`、`numpy`、`pyarrow`）改为函数内 import。
...
|---|---|---|
| L-1 依赖瘦身 | -2~-6s | -2~-6s |
| L-2 Electron require | -200ms | -2.2~-6.2s |
| L-3 ChromaDB 预热 | 首次 -200ms | -2.2~-6.2s |
| L-4 V8 Snapshot | -150ms | -2.35~-6.35s |
| L-5 ONNX 转换 | -50ms/次 | -2.35~-6.35s |
| **典型场景** | 3-6min → **110-180s** | 1.5-3x |
...
### 7.2 三段式可行性结论（v2.0 复核）

> [!success] 保守可达：3-6min → 30-60s（3-6x）
> 仅靠 **M-1（哈希兜底）+ M-2（QQ 超时调小）+ M-6（进度反馈）** 即可达成。这部分有 100% 把握，改动范围小，风险低。

> [!tip] 进阶可达：30-60s → 10-20s（再 3-6x）
> 需要 **U-1（QQ 非阻塞）+ U-2（并行化）+ U-3（懒加载）+ U-7（附件 import 懒加载）+ U-5（Electron 并行）**。架构级改造，需充分测试。

> [!warning] 10s 以内：确认可达（依赖关键项）
> **前提条件**（v2.0 已实测验证）：
...
> 4. **不存在必须同步完成的分钟级计算** —— ✅ 全量测试确认 27 failed 为环境依赖，非计算瓶颈
> 5. **附件处理链 import 懒加载（U-7）必须实施** —— ⚠️ 升级后 import 达 10.93s，需 -6.4s 才能进入 10s 窗口
...
| import 阶段（升级后） | 10.93s | 4.5s（U-7/U-8 懒加载） | -6.4s |
...
**结论**：理论上可达 **~9.5 秒**，在 10s 目标内。再配合 L-1 依赖瘦身 + L-4 V8 Snapshot 可进一步压到 **~8 秒**。
...
```mermaid
graph TD
    Step1[Step 1: 测量基线 ✅ 已完成<br/>import=10.93s + 全量 1278/27] --> Step2[Step 2: M-2 QQ 超时调小 30→5s<br/>+ M-6 进度反馈]
    ...
    Step5 --> Step6["Step 6: U-7 附件模块 import 懒加载<br/>升级后 -6.4s（最紧迫）"]
...
| U-7 附件懒加载首次上传延迟 | 高 | 低 | 懒加载缓存到模块单例（`_MARKITDOWN`），仅首次调用付费 |
| U-8 companion import 瘦身影响类型引用 | 低 | 高 | 用模块级 getter + `hasattr` 防护，编译期验证 |
...
- [tools/architecture_guard.py](file:///e:/Agent_reply/tools/architecture_guard.py)（B08 升级新增）
...
> [!success] 一键测量脚本已落地（v1.1 建议 → 已实施）
> - **一键测量**：[scripts/boot_trace.ps1](file:///e:/Agent_reply/scripts/boot_trace.ps1)（纯英文，避免 PS5 中文乱码）
> - **探针**：[scripts/boot_trace_probe.py](file:///e:/Agent_reply/scripts/boot_trace_probe.py)（复制 main._main() 的 import 序列）
> - **解析**：[scripts/boot_trace_parser.py](file:///e:/Agent_reply/scripts/boot_trace_parser.py)（生成中文 Markdown 报告）
> - **运行命令**：`powershell -ExecutionPolicy Bypass -File scripts\boot_trace.ps1`
> - **输出**：`importtime.log` + `Aerie_Obsidian_Vault/boot_trace_data.md`
> - **实测结果**：v1.1 基线 7.89s → 升级后 10.93s

```powershell
# 一键复测
powershell -ExecutionPolicy Bypass -File e:\Agent_reply\scripts\boot_trace.ps1
```
...
| architecture_guard | B08 架构守护工具，4 规则静态扫描 204 文件（升级后新增） |
...
| 2026-08-11 | v1.1 | Claude (GLM-5.2) | 补入 importtime 实测：import 7.89s；新增 U-7/U-8；修正根因排序为 10 阻塞点 |
| **2026-08-12** | **v2.0** | Claude (GLM-5.2) | **升级后全面修订**：补入 main↔Aerie-Model-X 同步后最新架构（B08/多通道/InProcess 保护）；更新实测数据（import 10.93s、全量 1278/27）；新增 §0 升级后现状；更新 U-7 为 -6.4s；10s 目标确认可达（9.5s） |

---

> [!quote] 结语（v2.0）
> 本文档 v2.0 基于 **2026-08-12 系统升级后** 的实测数据全面修订。升级（main↔Aerie-Model-X 同步 + B08 architecture_guard + 多通道身份 + InProcess 世界保护等）使系统更完善，但也使 import 阶段从 7.89s 升至 **10.93s**，启动优化紧迫性上升。
>
> 核心判断：**3-6 分钟 → 10 秒目标在技术上确认可达**（数学推演 ~9.5s）。关键实施顺序：U-1（QQ 非阻塞，-30s）→ U-2（并行化，-10~-30s）→ U-7（附件 import 懒加载，升级后 -6.4s，最紧迫）→ U-3（ONNX 懒加载）。
>
> 下一步行动建议：从 Phase 1 实施（M-2 QQ 超时调小 + M-6 进度反馈），这是风险最低、见效最快的切入点。完成 Phase 4（含 U-7）后预计进入 15-30s 区间，Phase 5-6 冲刺 10s 目标。
>
> 修订依据：[[plans/Aerie-Model-X分支升级执行计划]]（分支同步全记录）、[[boot_trace_data]]（升级后性能实测）。
