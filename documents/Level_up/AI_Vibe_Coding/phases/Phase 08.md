...
## 目标
持久化 cooldown、反馈、mute、postpone 与用户设置；保持兼容、可观测与可回滚。
...
## 文件范围
- 计划修改或演进：`core/proactive_judge.py`、`core/push_scheduler.py`
- 新文件仅限计划列明的模块、迁移和测试。
- 执行任务：[[Task 08-baseline]]

## 数据/API 合同
- Feature Flag：`proactive_delivery_v2`。
- 持久化 cooldown、反馈、mute、postpone 与用户设置。
...
## 提交边界
只提交 Phase 08 相关源码、测试、迁移与文档；不混入无关重构、格式化或构建产物。
