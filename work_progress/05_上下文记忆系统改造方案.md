...
- [x] 审计（艾莲）交付评审通过；Agent 安全扫描无新增高危 —— 艾莲审计报告 `work_progress/audit_report_P0-P3.md`（P0-P3 交付评审通过，有条件）；Agent 扫描 `work_progress/security_review_P0-P3.md` 无可用利用项
...
- [x] Agent 回归复核通过 —— 艾莲复核一致（伊塔侧 1228 passed/27 failed 与基线一致），Agent 正式会签记录待补签
...
- [ ] P1 上线稳定运行 ≥ 7 天
- [x] 审计确认 P2 项 ROI 可行（写入校验门 PoC 范围界定）—— 艾莲确认：PoC 一致率 100%、3436.7ms/次、0.054 元/千次，范围界定合理（B.8 + `tools/memory_validation_poc.py`）
...
- [ ] P0-P2 全部验收通过并稳定运行
- [x] `persona_timeline` 表设计评审（艾莲）通过 —— DDL 与附录 A.3.1 逐字段比对一致（审计报告 F 表无阻塞项）
- [ ] 用户确认"读取透明"边界条款（D-1 已确认，此处复核实施细节）

**过关点（验收标准）**

- [ ] §5 验收 2 复测（跨通道 ≥ 80% 连续性）通过 —— 离线代理实测：`tools/cross_channel_continuity_acceptance.py` 真实 LLM 判定，开启视图 B 后连续率 **100%**（对照 40%、注入率 100%），≥80% 门槛达成；QQ/桌面真实链路复测待运行时
...
- [x] 全量回归 + 性能无退化（平均 assemble 耗时对比基线）—— 1259 passed / 27 failed，与基线一致无新增；性能微基准 `tools/context_assembler_perf_check.py` 实测基线 0.87ms / 视图B 0.86ms，绝对增量 ≈ 0，新增字符 213 ≤ 500 预算（报告 `work_progress/context_assembler_perf_report.json`）
...
- [x] 艾莲交付评审 + Agent 安全扫描 —— 通过（`work_progress/audit_report_P0-P3.md` / `security_review_P0-P3.md`）
...
- [x] 艾莲交付评审 + Agent 安全扫描（P0 遗留，随 P1 交付一并会签）—— 通过
...
- 性能微基准 **PASS**：`tools/context_assembler_perf_check.py` 50 次采样、40 轮 seed——基线 mean 0.87ms / 视图 B mean 0.86ms，绝对增量 ≈ 0；视图 B 新增字符 213 ≤ 500 预算，不挤占 L0（报告 `work_progress/context_assembler_perf_report.json`）

**P3 剩余待办（运行时/用户验收）**：
- [x] §5 验收 2 离线代理复测：`tools/cross_channel_continuity_acceptance.py` —— 5 场景真实 LLM 判定，实验组连续率 **100%**（≥80% 门槛达成），对照组 40%，视图 B 注入率 100%；报告 `work_progress/cross_channel_continuity_acceptance_report.json`
- [ ] §5 验收 2 真实链路复测（QQ/桌面，开启 `multi_channel_identity_v1` 后运行时实测）
- [ ] 多端自我叙事实测：伊塔能主动引用跨端时间线事件（用户验收）
- [x] Agent 安全扫描（interim）—— `work_progress/security_review_P0-P3.md`：无可用利用项
- [x] 艾莲交付评审（会签 P0-P3 交付）—— 通过（有条件）：F1 迁移落账已由真实启动闭环（009/010 于 2026-08-12T19:30:55 在 data/aerie.db 完成）；F2 真实链路复测待用户运行时；F3/F4 观察项见审计报告
