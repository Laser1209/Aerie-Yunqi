"""byted-bp-cdn-pagesdeploy skill (cloud) — 字节边缘 Pages 部署 / BytePlus Pages.

Block-5B scaffold. Cloud-based skill that requires external
credentials (env: `BYTEPAGES_TOKEN`). When the env var is
...
def run(args: dict) -> dict:
    """Skill entry point. ``args`` is a free-form dict; key: 'project_dir'."""
    args = args or {}
    project_dir_value = args.get("project_dir")
    if not project_dir_value:
        return {"error": "missing project_dir", "provider_hint": PROVIDER_HINT}

    if not os.getenv('BYTEPAGES_TOKEN'):
        return {
            "status": "stub",
            "error": f"credential_missing: env {ENV_VAR!r} not set",
            "provider_hint": PROVIDER_HINT,
            "read_only": READ_ONLY,
            "project_dir": str(project_dir_value)[:80],
        }
