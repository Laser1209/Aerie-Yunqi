...
依赖列只使用完整批次 ID、G0-G6 阶段门或“无”；“产出门禁”只记录该批次通过后可关闭的阶段门；回滚列只使用 L0-L4 或“无状态变更”。

### 6.1 G0-G2：基线、控制面与对话主链

| 周 | 批次 | 目标 | 依赖 | Owner / Reviewer | 证据 | 产出门禁 | 回滚 | 初始状态 |
|---|---|---|---|---|---|---|---|---|
| W1 | P2-00-B01 | 冻结实际 HEAD、工作区归属、工具链和隔离数据目录 | 无 | ARCH / QA | BAS | - | 无状态变更 | Ready |
| W1 | P2-00-B02 | 重采测试、性能、安全和包内容基线 | P2-00-B01 | QA / ARCH | TST, PERF, SEC | - | 无状态变更 | Blocked |
| W1 | P2-00-B03 | 完成追踪矩阵、ADR、所有权和风险责任人 | P2-00-B01、P2-00-B02 | ARCH / PO | CON | G0 | 无状态变更 | Blocked |
...
- [ ] 实际 HEAD、分支、依赖锁、工具版本和隔离数据目录已冻结。
- [ ] 工作区所有未提交文件完成归属确认；运行数据和秘密明确排除。
- [ ] Python、Electron、E2E、性能、安全、包内容基线可复现。
- [ ] 13 个工作包追踪率 100%，ADR 无所有权或主链冲突。
- [ ] R-01 至 R-17 有责任人、控制措施和触发后操作。
- [ ] PO 批准进入 P2-01。
...
| 风险 | Owner | 最低自动化控制 | 必需证据 | 首选回滚 |
|---|---|---|---|---|
| R-01 平行主链 | ARCH | 架构守护和入口唯一性测试 | CON, TST | 停止合并，接回现有端口 |
| R-02 双写不一致 | CORE | 计数/顺序/owner 守恒和 dry-run | DATA | L0, L4 |
| R-03 重放副作用 | ARCH | idempotency、终态、ACK 后置和重放测试 | TST, OPS | 停 consumer 后安全重放 |
| R-04 资源泄漏/风暴 | WORLD, DESK | 有限队列、背压、隐藏停更和 soak | PERF, OPS | L0, L1 |
| R-05 配置漂移 | CORE, DESK | effective snapshot/revision 契约 | CON, TST | L3 |
| R-06 Persona 串线 | CORE | Actor/Channel/Persona 隔离测试 | DATA, TST | L0, L4 |
| R-07 主动消息打扰 | CORE, PO | 勿扰、上限、mute 和反馈回归 | TST, UX | L0 |
| R-08 外部依赖 | Adapter Owner, PO | 超时、熔断、预算、Mock 和文本降级 | OPS, PERF | L1 |
| R-09 插件/RPC 滥用 | SEC, WORLD | 本机认证、签名、版本握手和 capability allowlist | SEC, CON | L1, L2 |
| R-10 Renderer 权限 | DESK, SEC | sender/schema/path 校验和 CSP | SEC, TST | L0, L2 |
| R-11 包夹带数据 | REL, SEC | allowlist 与禁入清单扫描 | SEC, REL | 阻断发布、轮换暴露凭据 |
| R-12 版本不一致 | REL | 单一版本源和安装矩阵 | REL | L2 |
| R-13 工作区误覆盖 | ARCH, PO | G0 清点、显式暂存、逐文件 diff | BAS | 停止并保留现场 |
| R-14 范围挤压 | PO, ARCH | WIP、阶段门、Must/Should/Could | BAS | 延后非核心范围 |
| R-15 Windows 兼容 | DESK, WORLD, QA | DPI、休眠、Unicode、端口和杀软矩阵 | UX, OPS | L1 |
| R-16 低质知识 | CORE, PO | source/confidence/version 和人工确认 | DATA, TST | L0, L4 |
| R-17 模拟边界 | PO, DESK | `source=simulated` 合同和 UI 文案检查 | CON, UX | 暂停展示 |
...
当前只允许领取 `P2-00-B01`。完成后依次执行：

1. `P2-00-B01`：记录实际 HEAD、工作区归属、运行数据排除清单、工具版本和隔离数据目录。
2. `P2-00-B02`：在同一基线执行 Python、Electron、E2E、性能、安全和包内容采样。
3. `P2-00-B03`：建立需求追踪、ADR、风险责任和 Evidence 索引，提交 G0 审查。
4. G0 通过后将 `P2-01-B01` 与 `P2-11-B01` 置为 Ready；其余批次继续 Blocked。
