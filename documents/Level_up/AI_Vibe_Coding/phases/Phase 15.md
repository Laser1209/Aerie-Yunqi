...
- [test_phase15_world_dashboard_snapshot.py](file:///E:/Agent_reply/tests/test_phase15_world_dashboard_snapshot.py)
...
## 文件范围
- 已修改或演进：`electron/src/main.js`、`electron/src/preload.js`、`electron/src/world-dashboard-host.js`、`electron/src/renderer/index.html`、`electron/src/renderer/js/world-dashboard.js`、`electron/src/renderer/styles/world-dashboard.css`、`core/api_server.py`、`core/companion.py`、`core/world_image_candidates.py`
- 已新增：`tests/test_phase15_world_dashboard_host.py`、`electron/tests/world-dashboard-renderer.test.js`、`tests/test_phase15_world_dashboard_api.py`、`tests/test_phase15_world_dashboard_approval_handler.py`、`tests/test_phase15_world_dashboard_snapshot.py`
...
- Red：新增 `tests/test_phase15_world_dashboard_snapshot.py`、扩展 host/renderer 测试后，目标失败为 `/api/world/dashboard/snapshot` 404、`Companion` 缺少 `get_world_dashboard_snapshot`、`worldDashboard.getSnapshot`/preload IPC 缺失、renderer 缺少 `world-dashboard-summary/relationship/timeline/candidates` DOM。
- Green：`core/api_server.py` 新增只读 `/api/world/dashboard/snapshot` 白名单响应；`core/companion.py` 新增 `get_world_dashboard_snapshot()`，从 WorldPort 读取 public state、world/relationship/self-model 快照和 replay events，输出 `worldSummary`、`relationshipState`、`selfModel`、`actionTimeline`、`imageCandidates`，过滤 raw/prompt/token/credential 等敏感键；`electron/src/world-dashboard-host.js` 新增 `getSnapshot()` 并脱敏转发；`main.js`/`preload.js` 只暴露 `world-dashboard:get-snapshot` 窄 IPC；renderer 页面新增四块只读快照显示。
- 回滚：关闭 `world_sidecar_v1` 时 snapshot API/host 返回 disabled 且不调用 handler；删除 snapshot DOM/IPC 只回退到 status-only dashboard，不影响候选审批、聊天发布或 Phase 14 自动消费。
- 验证：`python -m py_compile core/api_server.py core/companion.py` 通过；`node --check electron/src/world-dashboard-host.js electron/src/main.js electron/src/preload.js electron/src/renderer/js/world-dashboard.js` 等价逐项通过；目标 snapshot `3 passed, 4 warnings`；Phase 14/15 相关 `23 passed, 4 warnings`；`python -m pytest tests -q` → `553 passed, 6 warnings`；Electron Node `25 passed`；`npm run check:all` 与 provider-key 工作区扫描通过。
